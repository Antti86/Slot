...
InputCtrl::InputCtrl(Board& brd)
	...
	BetMinusPos(brd.GetPos().x + 170.0f, brd.GetPos().y - 50),
	BetMinusBox(RectF(BetMinusPos.x, BetMinusPos.x + 40, BetMinusPos.y, BetMinusPos.y + 40)),
	BetPlusPos(brd.GetPos().x + 290.0f, brd.GetPos().y - 50),
...
void InputCtrl::Draw(Graphics& gfx) const
{
	...
	gfx.DrawRectLines(BetMinusBox, Colors::Green);
	gfx.DrawRectLines(BetPlusBox, Colors::Green);
}
